...
class Webhook(Plugin):
    ...
    def __init__(self, uri_endpoint: str, webhook_type: int, query_details: str):
        ...
        self._handlers = {1: self._rest, 2: self._graphql}
    ...
    def _rest(self, validator_info):
        r = requests.post(self._uri_endpoint, json=validator_info)
        log.debug(r.content)
    ...
    def _graphql(self, validator_info):
        ...
        r = requests.post(
            self._uri_endpoint,
            json={"query": self.query_details.replace("__message__", event_str)},
        )
        log.debug(r.content)
